...
public class XListView extends ListView implements OnScrollListener {
    ...
    public void onRefreshComplete() {
        if (mPullRefreshing == true) {
            ...
            String stringDateLong = getStringDateLong();
            editor.putString(mContext.getClass().getName() + ".lasttime",
                    stringDateLong);
            editor.commit();
            setRefreshTime(stringDateLong);
            ...
            if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
                if (mHeaderView.getVisiableHeight() != 0)
                    mHeaderView.setState(XListViewHeader.STATE_READY,
                            (float) mHeaderView.getVisiableHeight()
                                    / mHeaderViewHeight);
            } else {
                if (mHeaderView.getVisiableHeight() != 0)
    ...
    protected void onAttachedToWindow() {
        ...
        SharedPreferences preferences = mContext.getSharedPreferences(TAG,
                Context.MODE_PRIVATE);
        setRefreshTime(preferences.getString(mContext.getClass().getName()
                + ".lasttime", ""));
    }
    ...
    public void setOnInterceptTouchListener(boolean Is) {
        mIntercepter = Is;
    }
